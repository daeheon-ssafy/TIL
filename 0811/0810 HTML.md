...
* css 적용 방법
  ...
  1. inline : 관리하기 힘듦. for test
     ...
     ``` 
     <div style="background-color: red;"></dic>
     ```

  2. 내부 참조 방식 : 하나의 html에서만 적용. for study, test

     ```
     <styel> h1 { color: red; } </style>
     ```

  3. 외부 참조 방식 : css 정의를 파일 단위로 묶어서 필요한 html 문서 마다 적용이 가능, 유지 보수가 쉬움. 파일을 따로 만들어서 관리를 해야하기 때문에 css파일을 잘 챙겨야 한다.
...
* css 단위
  ...
  * vw(뷰포트 너비의 1%), vh, vmin(뷰포트에서 가로세로 중에서 짧은쪽의 1%), vmax
...
* CSS Position
  ...
  * relative : 자기 자신의 초기 위치를 기준으로 이동

  * absolute : static(자기 자신) 속성이 아닌 가장 가까운 부모 / 조상 요소를 기준으로 이동
  ...
  * sticky : relative 처럼 기본 배치 순서는 가지고 있음.
    ...
    * 부모의 영역이 화면 밖으며 벗어나면 다시 일반적인 흐름을 따르게 됨
